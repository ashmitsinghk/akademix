from functionalities import database, searchStudent, sendQR, editStudent
...
def viewStudent(admission_no):
    ...
    gender = str(lastRow[3])
    grade = str(lastRow[4])
    section = str(lastRow[5])
    dob = str(lastRow[6])
    contact = str(lastRow[7])
    email = str(lastRow[8])
    attendance = str(lastRow[9])
    motherName = str(lastRow[10])
    fatherName = str(lastRow[11])
    address = str(lastRow[12])
    ...
    value_gender = Label(root, text=gender)
    value_gender.place(x=150, y=140)
    value_grade = Label(root, text=grade)
    value_grade.place(x=150, y=160)
    value_section = Label(root, text=section)
    value_section.place(x=150, y=180)
    value_dob = Label(root, text=dob)
    value_dob.place(x=150, y=200)
    value_contact = Label(root, text=contact)
    value_contact.place(x=150, y=220)
    value_email = Label(root, text=email)
    value_email.place(x=150, y=240)
    value_attendance = Label(root, text=attendance)
    value_attendance.place(x=150, y=260)
    value_motherName = Label(root, text=motherName)
    value_motherName.place(x=150, y=280)
    value_fatherName = Label(root, text=fatherName)
    value_fatherName.place(x=150, y=300)
    value_address = Label(root, text=address)
    value_address.place(x=150, y=320)
